...
import (
	...
	"github.com/yangl900/acr-tokenman/auth"
...
// +kubebuilder:rbac:groups=batch.tutorial.kubebuilder.io,resources=secrets,verbs=get;list;watch;create;update;patch;delete
...
func (r *SecretReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if secret.Type == v1.SecretTypeDockerConfigJson {
		...
		var ok bool
		if clientID, ok = secret.Annotations["tokenman/clientID"]; !ok {
			log.Info("Secret does not have client id annotation, skip")
			return ctrl.Result{}, nil
		}

		if acr, ok = secret.Annotations["tokenman/acr"]; !ok {
			log.Info("Secret does not have acr annotation, skip")
			return ctrl.Result{}, nil
		}

		log.Info("Found specified client ID", "client_id", clientID)
